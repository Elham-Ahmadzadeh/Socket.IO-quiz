...
const sendNextQuestion = async function () {
  ...
  try {
    ...
    io.in('viewerRoom').emit('sendOnlyQuestion', `Question${index+1}: ${questionAndAnswers.question}`)
...
io.on('connect', (socket) => {
  ...
  socket.on('checkAnswers', (ans) => {
    ...
    if (ans === correctAnswer) {
      ...
      io.in('viewerRoom').emit('correctAnswer', `${ans}(wrong) The correct answer is ${correctAnswer}`)
      sendNextQuestion()
    }
    if (index === 5) {
